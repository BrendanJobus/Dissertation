...
def train_FedDC(data_obj, act_prob,n_minibatch,
    ...
    n_clnt = data_obj.n_client   # number of clients
    clnt_x = data_obj.clnt_x; clnt_y=data_obj.clnt_y # seperating out the client x and y's
    ...
    if (trial) or (not os.path.exists('%sModel/%s/%s/ins_avg_%dcom.pt' %(data_path, data_obj.name, suffix, com_amount))):
        ...
        ###### From here on is where we train the model itself
        for i in range(saved_itr+1, com_amount):
            ...
            for clnt in selected_clnts:   ###### Training each client
                ...
                delta_param_curr = curr_model_par - cld_mdl_param   # theta i+ - theta i
                parameter_drifts[clnt] += delta_param_curr   # hi + (theta i+ - theta i)
